...
<template>
  <view
    class="cannonui-picker__list"
    @touchstart="onTouchStart"
    @touchmove="onTouchMove"
    @touchend="onTouchEnd"
  >
    <view
      class="cannonui-picker-roller"
      ref="roller"
    ...
    >
      ...
      >
        <!-- 3D 效果 -->
        <view
          class="cannonui-picker-roller-item"
          :class="{ 'cannonui-picker-roller-item-hidden': isHidden(index + 1) }"
        ...
        <!-- 平铺 -->
        <view
          class="cannonui-picker-roller-item-tile"
    ...
    <view
      class="cannonui-picker-roller-mask"
      :style="maskStyles"
    ></view>
  </view>
</template>
...
import type { PickerOption } from '@/cmpStore/Pick/vue/usePicker';
import { clamp, preventDefault, pxCheck } from '@/utils/util';
import createComponent from '@/utils/vue_component';
import { useTouch } from '@/utils/vue_useTouch';
import {
  PropType,
  computed,
  onMounted,
...
const { componentName, create } = createComponent('pickcolumn');
export default create({
  ...
  setup(props, { emit }) {
    ...
    const maskStyles = computed(() => {
      return {
        backgroundSize: `100% ${
          ((+props.visibleOptionNum - 1) * +props.optionHeight) / 2
        }px`
      };
    });
    ...
    const onTouchStart = (event: TouchEvent) => {
      ...
      if (moving.value) {
        ...
        if (props.threeDimensional) {
          ...
          state.scrollDistance = -Math.abs(
            (endDeg / state.rotation - 1) * +props.optionHeight
          );
        } else {
          console.log(+transform, 'transform');

          state.scrollDistance = +transform
    ...
    const setMove = (move: number, type?: string, time?: number) => {
      ...
      if (type === 'end') {
        ...
        let deg = `${
          (Math.abs(Math.round(endMove / +optionHeight)) + 1) * state.rotation
        }deg`;
        ...
        if (minDeg < deg && deg < maxDeg) {
          ...
          state.currIndex =
            Math.abs(Math.round(updateMove / +optionHeight)) + 1;
        }
      }
    };
    ...
    const modifyStatus = (type: boolean) => {
      const { column } = props;
      let index = column.findIndex(
        (columnItem) => columnItem.value == props.value
      );
